...
var wheelOpt=supportsPassive?{passive:false}:false;var wheelEvent='onwheel'in document.createElement('div')?'wheel':'mousewheel';function disableScroll(){if($('body').hasClass('smooth-scroll')){let aliothSmoother=ScrollSmoother.get();aliothSmoother.paused(true);}else{window.addEventListener('DOMMouseScroll',preventDefault,false);window.addEventListener(wheelEvent,preventDefault,wheelOpt);window.addEventListener('touchmove',preventDefault,wheelOpt);window.addEventListener('keydown',preventDefaultForScrollKeys,false);}}
function enableScroll(){if($('body').hasClass('smooth-scroll')){let aliothSmoother=ScrollSmoother.get();aliothSmoother.paused(false);}else{window.removeEventListener('DOMMouseScroll',preventDefault,false);window.removeEventListener(wheelEvent,preventDefault,wheelOpt);window.removeEventListener('touchmove',preventDefault,wheelOpt);window.removeEventListener('keydown',preventDefaultForScrollKeys,false);}}
var siteHeader=$('.site-header'),page=$('main'),body=$('body'),mouseCursor=$('#mouseCursor'),cursorDark,cursorLight,cursorY,scrollSmoother,$cursorInner=' <div class="mouse-cursor-icon"></div><svg height="100%" width="100%" viewbox="0 0 100 100"><circle class="main-circle" cx="50" cy="50" r="40" /></svg><span class="mouse-cursor-text"></span>';mouseCursor.append($cursorInner);var matchMedia=gsap.matchMedia(),isPhone='(max-width: 450px)',isTablet='(min-width: 450px) and (max-width: 900px)',isDesktop='(min-width: 900px)';function naylaMouseCursor(init){if(init){let cursorText=mouseCursor.children('.mouse-cursor-text').wrapInner('<span></span>'),cursorIcon=mouseCursor.children('.mouse-cursor-icon'),yperc,cursorActive;gsap.set(mouseCursor,{xPercent:-50,yPercent:-50});let xTo=gsap.quickTo(mouseCursor,"x",{duration:0.6,ease:"power3"}),yTo=gsap.quickTo(mouseCursor,"y",{duration:0.6,ease:"power3"});function icko(e){xTo(e.clientX);yTo(e.clientY);cursorY=e.clientY}
window.addEventListener("mousemove",icko);}
function defaultHover(targets){$(targets).on('mouseenter',function(){mouseCursor.addClass('hover-size')})
...
function iconHover(targets){$(targets).on('mouseenter',function(){let $this=$(this),icon=$this.data('cursor-icon');icon==null?icon='arrow_outward':'';cursorIcon.attr('data-icon',icon);mouseCursor.addClass('hover-size');mouseCursor.addClass('hover-icon');})
...
function cursorDrag(action){if(action==='hover'){mouseCursor.removeClass('press')
mouseCursor.append('<div class="multi-icon"></div>')
mouseCursor.addClass('hover-size');mouseCursor.addClass('multi');}
if(action==='press'){mouseCursor.addClass('press')}
if(action==='leave'){mouseCursor.removeClass('hover-size hover-icon multi press');$('.multi-icon').remove();}}
...
function pageLoader(){let perc=$('.page-loader-percentage'),totImages,loadedImages,percentage,minTime=5,img=$('.page-loader-logo').children('img'),caption=$('.page-loader-caption'),countLabels=[0];img.length?img.clone().addClass('clone').insertAfter(img):'';countLabels.push(Math.floor(gsap.utils.random(20,30)));countLabels.push(Math.floor(gsap.utils.random(40,60)));countLabels.push(Math.floor(gsap.utils.random(60,80)));countLabels.push(100);perc.append('<div class="page-loader-count"></div>');countLabels.forEach(element=>$('.page-loader-count').append('<span class="count_'+element+'">'+element+'</span>'));let tl=gsap.timeline({delay:.1,id:'pageLoaderAnimation',onStart:()=>{$('html').addClass('loading')},onComplete:()=>{if(winLoaded==true){setTimeout(function(){gsap.to('.page-loader > div',{opacity:0})
gsap.to('.page-loader',{height:0,delay:0,duration:1,ease:'expo.inOut',onStart:()=>{setTimeout(function(){ScrollTrigger.enable()
$('html').removeClass('loading')},500)},})},100)}else{$(window).on('load',function(){setTimeout(function(){gsap.to('.page-loader > div',{opacity:0})
gsap.to('.page-loader',{height:0,delay:0,duration:1,ease:'expo.inOut',onStart:()=>{setTimeout(function(){ScrollTrigger.enable()
$('html').removeClass('loading')},500)},})},100)})}}});if(caption.length){new SplitText(caption,{type:'lines, chars',linesClass:'capt_line',charsClass:'capt_char'})
let lines=caption.find('.capt_line'),chars=caption.find('.capt_char');lines.clone().addClass('clone').appendTo(caption)
gsap.from(chars,{yPercent:100,duration:.75,stagger:0.02,delay:.5,ease:'expo.out',})}
...
tl.to(capt,{width:wido,duration:1,ease:'expo.inOut'},'label_'+snapPerc);}}).done(function(){});};function smoothScroll(){if(body.hasClass('smooth-scroll')){$('#page').wrap('<div id="smooth-wrapper"></div>');$('#primary').wrap('<div id="smooth-content"></div>');if((!siteHeader.hasClass('fixed'))&&(!siteHeader.hasClass('sticky'))){siteHeader.prependTo('#smooth-content')
$('.menu-overlay').prependTo('#smooth-content')}
$('#footer').appendTo('#smooth-content')
var smoother=ScrollSmoother.create({wrapper:'#smooth-wrapper',content:"#smooth-content",smooth:1,normalizeScroll:false,ignoreMobileResize:true,effects:false,preventDefault:true,});}}
...
function naylaHeader(){var headerHeight=siteHeader.outerHeight(),stickyTargets=$('.hide-sticky'),logo=$('.site-logo'),stickyLogo=$('.sticky-logo'),allInstances=ScrollTrigger.getAll(),headerPadding=siteHeader.css('paddingTop'),resetDelay=1,fixed,sticky,dynamic;function animateLogos(direction){if(stickyLogo){let tl=gsap.timeline();if(direction=='play'){tl.to(logo,{opacity:0,duration:0.35,onComplete:()=>{logo.hide()}})
...
function resetHeader(stAnim){animateLogos('reverse');if(dynamic!=true){if(sticky){gsap.to(siteHeader,{paddingTop:headerPadding,paddingBottom:headerPadding,ease:'power2.out',duration:.4,onComplete:()=>{siteHeader.removeClass('sticked');gsap.set(siteHeader,{clearProps:'all'})}})}else if(fixed){siteHeader.removeClass('sticked');}}else{siteHeader.removeClass('sticked');}}
var delayedCall=gsap.delayedCall(resetDelay,resetHeader);delayedCall.kill();function fixedHeader(){ScrollTrigger.addEventListener('refresh',()=>{var fixedStart='top top';allInstances.forEach(function(st){if((st.pin!=null)&&(st.start<=0)){fixedStart='top+='+st.end+' top';}})
...
siteHeader.hasClass('fixed')?fixedHeader():'';function dynamicHeader(){var fixedStart;allInstances.forEach(function(st){if((st.pin!=null)&&(st.start<=0)){fixedStart='top+='+st.vars.end+' top';ScrollTrigger.create({trigger:'body',pin:siteHeader,start:'top top',end:fixedStart,id:'header_pin'})}})}
siteHeader.hasClass('dynamic')?dynamicHeader():'';function stickyHeader(){let stickyStart='top+=1000 center',stickyReset='top top',topPin=[];var stickyAnim=ScrollTrigger.create({trigger:"body",start:'top top',end:'bottom bottom',id:'stickyAnim',onUpdate:function(self){if(self.direction==-1){gsap.to(siteHeader,{y:'0%'})}else if(self.direction==1){gsap.to(siteHeader,{y:'-100%'})}},});stickyAnim.disable();allInstances.forEach(function(st){if((st.pin!=null)&&(st.start<=0)){topPin.push(st)}})
if(topPin){stickyStart='top+='+(600+topPin[0].end)+' center';stickyReset='top+='+topPin[0].end+' top';let pin=ScrollTrigger.create({trigger:'body',pin:siteHeader,start:'top top',end:stickyReset})
ScrollTrigger.create({trigger:'body',start:stickyReset,end:stickyStart,onLeave:()=>{gsap.set(siteHeader,{y:'-100%'})
stickyAnim.enable();siteHeader.addClass('sticked');animateLogos('play')},onLeaveBack:()=>{siteHeader.removeClass('sticked')
gsap.set(siteHeader,{clearProps:'all'})},onEnter:()=>{delayedCall.kill();}})}else{ScrollTrigger.create({trigger:'body',start:'top top',end:'top+=1000 center',onLeave:()=>{gsap.to(siteHeader,{y:'-100%'})
stickyAnim.enable();siteHeader.addClass('sticked');animateLogos('play')},onLeaveBack:()=>{delayedCall.restart(true)},onEnter:()=>{delayedCall.kill();}})}}
siteHeader.hasClass('sticky')?stickyHeader():'';var resetNavigation='';function classicNavigation(){let nav=$('#site-navigation.classic'),overlay=$('.menu-overlay'),headerHeight=siteHeader.outerHeight(),subBg=nav.data('sub-background');if(nav.hasClass('one-page')){nav.children('ul').children('li').each(function(){let $this=$(this),target=$this.children('a').attr('href');$this.find('a').on('click',(e)=>{e.preventDefault()})
$this.find('a').addClass('scroll-button');$this.find('a').attr('data-scroll-to',target);})}
let hsl=gsap.utils.splitColor(subBg,true),lightness;lightness=hsl[hsl.length-1];if((lightness<=50)&&(lightness!=0)){overlay.addClass('dark')}else{overlay.addClass('light')}
function headerLayout(reset){if(reset){siteHeader.removeClass('menu-open');if(headerDark){siteHeader.removeClass('light')
siteHeader.addClass('dark')}else if(headerLight){siteHeader.removeClass('dark')
siteHeader.addClass('light')}}else{if((lightness<=50)&&(lightness!=0)&&(headerDark==true)){siteHeader.removeClass('dark')
siteHeader.addClass('light')}
if((lightness>50)&&(lightness!=0)&&(headerLight==true)){siteHeader.removeClass('light')
siteHeader.addClass('dark')}}}
var headerReset=gsap.delayedCall(.45,headerLayout,[true]);headerReset.kill();if(nav.length){resetNavigation=document.getElementById("site-navigation").innerHTML;nav.each(function(){let $this=$(this),menu=$this.children('ul'),menuItems=menu.children('li');menuItems.each(function(){let menuItem=$(this),link=menuItem.children('a');link.wrapInner('<span class="hov_span" data-text="'+link.text()+'"></span>');if(menuItem.hasClass('has-children')){var subMenuIn=gsap.timeline({onStart:()=>{subMenuOut.pause();},}),subMenuOut=gsap.timeline({onStart:()=>{subMenuIn.pause();},onComplete:()=>{subMenuWrapper.removeClass('active')}}),subMenuWrapper=menuItem.children('.sub-menu-wrapper'),subChilds=subMenuWrapper.children();subMenuWrapper.css('top',menuItem.offset().top+menuItem.outerHeight())
menuItem.on('mouseenter',function(i){headerReset.kill();headerLayout(false)
subMenuWrapper.addClass('active')
siteHeader.addClass('sub-active')
let wrapperHeight=subMenuWrapper.outerHeight();subMenuIn.restart();gsap.to(overlay,{height:wrapperHeight+headerHeight,duration:.7,ease:'expo.out'})
subMenuIn.fromTo(subChilds,{x:100,y:0,opacity:0},{x:0,y:0,opacity:1,stagger:0.1,duration:1.5,ease:'expo.out'},0.2)})
menuItem.on('mouseleave',function(){headerReset.restart(true)
subMenuOut.restart();gsap.to(overlay,{height:0,duration:.7,ease:'expo.inOut'})
subMenuOut.to(subChilds,{x:-50,opacity:0,duration:.4,ease:'expo.out'},0)})}})})}}
classicNavigation();function menuItemAnims(menu,direction){menu.each(function(){let items=$(this).children('li'),targets=[items.children('a').find('.mt_char'),items.children('.sub-toggle')],val=items.outerHeight(),stg=0.02;$(this).hasClass('sub-menu')?stg=0.01:'';if(direction==='in'){gsap.fromTo(targets,{y:val},{y:0,stagger:stg,duration:1.5,ease:'expo.out',overwrite:true,onStart:()=>{$(this).addClass('active');},})}else if(direction==='out'){var animOut=gsap.fromTo(targets,{y:0},{y:-1*val,stagger:0.005,overwrite:true,duration:.5,ease:'expo.in',onStart:()=>{$(this).addClass('animating')},onComplete:()=>{$(this).removeClass('active')
$(this).removeClass('animating')}})}})}
function fullscreenNavigation(){let nav=$('#site-navigation.fullscreen_menu'),wrapper=nav.children('.fullscreen-menu-wrapper'),mainMenu=nav.find('.main-menu'),items=mainMenu.find('li a'),hasChildren=mainMenu.find('li.has-children');items.addClass('menu-link');if(nav.length){hasChildren.each(function(){let childLength=$(this).find('li').not('.sub-menu-title').length;$(this).append('<span class="sub-toggle">('+childLength+')</span>');})
...
wrapper.css('paddingTop',headerHeight+50);hasChildren.children('.sub-toggle').on('click',function(){let $this=$(this).parent('li'),parentMenu=$this.parent('ul'),subMenu=$this.find('.sub-menu');subClose.addClass('active')
...
subClose.on('click',function(){let outMenu=nav.find('ul.menu.active'),inMenu=outMenu.parent('div').parent('li').parent('ul');menuItemAnims(outMenu,'out');gsap.delayedCall(.5,menuItemAnims,[inMenu,"in"]);if(inMenu.hasClass('main-menu')){subClose.removeClass('active')}})}}
fullscreenNavigation();function menuToggle(targetMenu){var menuToggle=$('.menu-toggle'),overlay=$('.menu-overlay'),text=menuToggle.find('.toggle-text'),closeText=text.attr('data-close-text'),bgColor=overlay.css('backgroundColor');text.wrapInner('<span data-close-text="'+closeText+'"></span>');var headerReset=gsap.delayedCall(1,headerLayoutChange,[false,false,false,true]);headerReset.kill();let navTweens=gsap.getTweensOf(targetMenu.find('*').not('.main-menu *, .nayla-button, .nayla-button *')),i=0,tltl=gsap.timeline({id:'fs_menu_tl'});for(i=0;i<navTweens.length;i++){navTweens[i].parent.pause()
navTweens[i].parent.scrollTrigger.kill()
tltl.add(navTweens[i],(0+i/10))}
tltl.pause();menuToggle.on('click',function(){var clicks=$(this).data('clicks'),mainMenu=targetMenu.find('.main-menu');if(clicks){enableScroll();let activeMenu=targetMenu.find('ul.menu.active');targetMenu.find('.sub-close').removeClass('active');$('html').hasClass('loading')?'':headerReset.restart(true);menuItemAnims(activeMenu,'out');tltl.timeScale(3)
tltl.reverse();gsap.killTweensOf(overlay);gsap.to(overlay,{height:0,duration:.7,delay:.5,ease:'expo.inOut',onComplete:()=>{menuToggle.removeClass('active');targetMenu.removeClass('active');}})}else{disableScroll();menuToggle.addClass('active')
targetMenu.addClass('active');siteHeader.addClass('menu-open');headerReset.kill();headerLayoutChange(false,bgColor,false,false)
menuItemAnims(mainMenu,'in');gsap.killTweensOf(overlay);gsap.to(overlay,{height:'100vh',duration:1,delay:0,ease:'expo.out',})
tltl.timeScale(1)
tltl.restart(true);}
$(this).data("clicks",!clicks);})}
let nav=$('#site-navigation'),items=nav.find('li'),mainMenu=$('#page'),isClassic;if((nav.hasClass('classic'))&&(!nav.hasClass('one-page'))){isClassic=true;}
matchMedia.add({isMobile:"(max-width: 450px)"},(context)=>{let{isMobile}=context.conditions;if(isClassic){gsap.set('.menu-overlay',{clearProps:'all'})
document.getElementById("site-navigation").innerHTML=resetNavigation;$('.sub-menu').unwrap();nav.removeClass('classic')
nav.addClass('fullscreen_menu temp_fullscreen section');nav.wrapInner('<div class="wrapper fullscreen-menu-wrapper"><div class="c-col-12"></div></div>');fullscreenNavigation();let headerHeight=siteHeader.outerHeight();$('.fullscreen-menu-wrapper').css('paddingTop',headerHeight);}
return()=>{if(isClassic){document.getElementById("site-navigation").innerHTML=resetNavigation;gsap.set(nav,{clearProps:'all'})
nav.addClass('classic')
nav.removeClass('fullscreen_menu temp_fullscreen section');classicNavigation();}}});menuToggle(nav);}
...
if((this.scrub)&&(!this.pin)){this.scroll.scrollTrigger.scrub=1;this.scroll.scrollTrigger.start='top bottom';this.scroll.scrollTrigger.end='top top'}
...
tl.fromTo(this.target,this.fromOptions,this.options);this.animOut==true?this.tl.to(this.target,this.out):'';}};function naylaGeneralAnims(target){target.each(function(){let $this=$(this);new naylaGeneralAnimations($this)})}
...
class naylaImageAnimation{DOM={el:null,image:null};animations=['slideLeft','slideRight','slideUp','slideDown','blockUp','blockLeft','blockRight','blockDown'];defaults={duration:1,delay:0,stagger:0,ease:'expo.out',};from={yPercent:0,}
scroll={scrollTrigger:{trigger:null,scrub:null,pin:null,start:'top bottom'}}
out={yPercent:null,stagger:null,duration:null,ease:'expo.out'}
constructor(DOM_el,options,fromOptions,scroll,out){this.DOM.el=DOM_el;this.duration=$(this.DOM.el).data('duration');this.delay=$(this.DOM.el).data('delay');this.scrub=$(this.DOM.el).data('scrub');this.scroll.scrollTrigger.trigger=this.DOM.el;this.classes=this.DOM.el.attr('class').split(' ');let animations=this.animations,classes=this.classes;const activeAnimation=classes.filter(function(obj){return animations.indexOf(obj)!==-1;});if(classes.includes('pin-anim')){this.scroll.scrollTrigger.pin=true
this.scroll.scrollTrigger.scrub=true
this.scroll.scrollTrigger.start='center center'}
this.target=$(this.DOM.el);this.img=$(this.DOM.el).children('img');this.scaleimage=false;classes.includes('imgScale')?this.scaleimage=true:'';if(activeAnimation[0]==='slideLeft'){this.from.clipPath='inset(0% 0% 0% 100%)';this.defaults.clipPath='inset(0% 0% 0% 0%)';this.defaults.duration=2;}
if(activeAnimation[0]==='slideRight'){this.from.clipPath='inset(0% 100% 0% 0%)';this.defaults.clipPath='inset(0% 0% 0% 0%)';this.defaults.duration=1.25;}
if(activeAnimation[0]==='slideUp'){this.from.clipPath='inset(100% 0% 0% 0%)';this.defaults.clipPath='inset(0% 0% 0% 0%)';this.defaults.duration=2;}
if(activeAnimation[0]==='slideDown'){this.from.clipPath='inset(0% 0% 100% 0%)';this.defaults.clipPath='inset(0% 0% 0% 0%)';this.defaults.duration=1.25;this.defaults.ease='power3.inOut';}
...
activeAnimation[0]==='blockUp'?this.from.height='100%':'';this.defaults.height='0%';this.defaults.duration=1;this.defaults.ease='expo.inOut';}
this.delay==null?this.delay=this.defaults.delay:'';this.duration==null?this.duration=this.defaults.duration:'';this.scrub==null?this.scrub=false:'';this.options=Object.assign(this.defaults,options);this.fromOptions=Object.assign(this.from,fromOptions);this.scroll=Object.assign(this.scroll,scroll);this.options.delay=this.delay;this.options.duration=this.duration;this.scroll.scrollTrigger.scrub=this.scrub
if(classes.includes('anim-out')){this.out.stagger=this.options.stagger;this.out.duration=this.options.duration;this.out=Object.assign(this.out,out);}else{this.out=null;}
...
this.out!=null?tl.to(this.target,this.out):'';}};function naylaImageAnims(){$('.has-anim-image').each(function(){let $this=$(this);new naylaImageAnimation($this);})}
...
this.tl.fromTo(this.target,this.fromOptions,this.options);this.animOut==true?this.tl.to(this.target,this.out):'';this.tl.eventCallback("onComplete",()=>{$(this.DOM.el).addClass('is-inview')
this.scrub!=true&&!this.noRevert?this.split.revert():'';this.progress=1;});}};function naylaTextAnims(){let hasAnim=$('.has-anim-text'),exclude='ul, li, a';hasAnim.not(exclude).each(function(){let $this=$(this)
...
if((header.hasClass('fullscreen-image'))||(header.hasClass('half-image'))||(header.hasClass('tall'))){gsap.to(image.find('img'),{yPercent:30,ease:'none',scrollTrigger:{trigger:header,start:'top top',end:'bottom top',scrub:1}});}
...
if($this.data('barba-namespace')!=='project-gallery-horizontal'||$this.data('barba-namespace')!=='project-gallery-vertical'){let tl=gsap.timeline({scrollTrigger:{trigger:nextProj,scrub:true,start:'bottom bottom',end:'bottom top',}});tl.fromTo(nextTitle,{opacity:0,yPercent:100},{opacity:1,yPercent:-50,},0)
ScrollTrigger.create({trigger:nextProj,start:'bottom bottom',end:'bottom top',pin:true,pinSpacing:false,})}}
...
let $this=$(this),image=$this.find('img'),metas=$this.find('.grid-project-meta').children('div'),title=$this.find('.grid-project-title'),category=$this.find('.grid-project-category'),titleText=title.text();$this.addClass('project_'+i);$this.attr('data-index',i)
...
let $this=$(this),image=$this.find('.project-image'),findImage='.image_'+i,img=image.find('img').attr('src'),secondary=$this.find('a');$this.attr('data-index',i);$this.prepend('<span class="project-index">'+i+'</span>')
...
return()=>{}});})}
function showcase3dTitles(){let showcase3dTitles=$('.showcase-3d-titles');showcase3dTitles.each(function(){let $this=$(this),projectsWrap=$this.find('.s3t-projects-wrap'),project=$this.find('.showcase-project'),imagesWrap=$this.find('.s3t-images'),projectLength=project.length,rotateX=360/projectLength,fontSize=parseInt($this.find('.project-title').css('font-size'));imagesWrap.addClass('transition--media');project.each(function(i){let $this=$(this),image=$this.find('.project-image');$this.find('a').addClass('sc-link');$this.find('a').on('click',()=>{$this.off('mouseleave')})
$this.data('index',i)
image.addClass('image_'+i).appendTo(imagesWrap)
$(this).css('--rotateX',-1*rotateX*i+"deg")
$(this).addClass('active-title_'+i)
$this.on('mouseenter',()=>{if($this.hasClass('active')){imagesWrap.addClass('active')}})
$this.on('mouseleave',()=>{if($this.hasClass('active')){let image='.image_'+$this.data('index');imagesWrap.removeClass('active')}})})
$this.css('--transformZ',projectLength*fontSize/6.4+"px")
let endo=$(window).outerHeight()*projectLength/3;gsap.to(projectsWrap,{rotateX:"360",ease:"none",scrollTrigger:{trigger:$this,pin:true,start:"top top",end:endo,scrub:1,repeat:-1,onUpdate:(self)=>{let prog=self.progress*projectLength,snap=gsap.utils.snap(1)
...
$this.find('.active-title_'+snap(prog)%projectLength).nextAll().removeClass('active');let activeProj=$('.showcase-project.active'),image='.image_'+activeProj.data('index');$('.project-image').removeClass('active')
...
function showcaseFullscreenSlideshow(){var fullscreenSlideshow=$('.showcase-fullscreen-slideshow');fullscreenSlideshow.each(function(){let fsSlideshow=$(this),projects=fsSlideshow.find('.showcase-project'),images=fsSlideshow.find('.fs-images-slider'),footer=fsSlideshow.find('.showcase-footer'),current=fsSlideshow.find('.sfs-current'),total=fsSlideshow.find('.sfs-total'),headerChange,footerChange;images.addClass('transition--media')
current.html('01')
...
function showcaseMinimalList(){var mainWrap=$('.showcase-minimal-list');mainWrap.each(function(){var mainWrap=$(this),projectsWrap=mainWrap.find('.sml-projects-wrap'),pwHeight=projectsWrap.outerHeight(),project=projectsWrap.find('.showcase-project'),projectsLength=project.length,images=mainWrap.find('.sml-images'),overlay=mainWrap.find('.sfc-overlay'),animation=mainWrap.data('animation'),colorAnim;images.addClass('transition--media')
colorAnim=true;if(!mainWrap.hasClass('animate-colors')){colorAnim=false}
project.each(function(i){let $this=$(this),title=$this.find('.project-title'),image=$this.find('.project-image');animation?title.addClass('has-anim-text '+animation):'';$this.find('a').addClass('sc-link');if(colorAnim==true){projectColorSettings($this);}
$this.attr('data-index',i)
image.addClass('image_'+i).appendTo(images);$this.addClass('project_'+i);})
let end=window.innerHeight*5;gsap.to(projectsWrap,{y:pwHeight*-1,ease:"none",scrollTrigger:{trigger:projectsWrap,start:"top top",end:end,pin:mainWrap,scrub:true,onUpdate:(self)=>{let prog=self.progress*projectsLength,snap=Math.floor(prog),findProj=$('.project_'+snap),findImg='.image_'+findProj.data('index'),path=findProj.data('mask'),bgColor=findProj.data('background-color'),elementsColor=findProj.data('elements-color');project.removeClass('active')
findProj.addClass('active');images.find('.project-image').hide();images.find('.project-image').removeClass('active');$(findImg).show();$(findImg).addClass('active');layoutChange(overlay,findProj,projectsWrap.find('a'))
changeHeaderStatus()}}})})}
...
$this.find('a').addClass('cursor-icon');$this.find('a').attr('data-cursor-icon','arrow_outward')
...
var tl=gsap.timeline({scrollTrigger:{trigger:mainWrap,pin:true,scrub:1,end:speed,markers:false,id:'showcaseCarouselScroll',onUpdate:(self)=>{if(self.progress<=0.0001){projects.removeClass('active');mainWrap.find('.project_0').addClass('active');}else if(self.progress>=0.0001){projects.each(function(i){let $this=$(this),ofLeft=$this.offset().left,windowWidth=$(window).outerWidth(),deadLine=(windowWidth/2)-$this.outerWidth(),currentTransform=scWrap.css('transform'),gimmeThat=currentTransform.split(" ")[4].replace(/\,/g,'');if((ofLeft<(windowWidth/2))&&(ofLeft>deadLine)){$this.addClass('active');}else{$this.removeClass('active');}});}
let activeProject=mainWrap.find('.showcase-project.active'),slideGo=activeProject.data('index');curr.html(slideGo+1)
...
let $this=$(this),image=$this.find('.project-image'),img=$this.find('img');if(horizontal&&(i%2===0)){$this.appendTo('.sfg_grid_row_bottom')}else{$this.appendTo('.sfg_grid_row_top')}
$this.attr('data-index',i)
for(let i=0;i<=5;++i){img.clone().appendTo($this.find('.project-image'))}
img=$this.find('img');img.each(function(i){i++
gsap.set($(this),{opacity:'0.'+i})})
...
projects.each(function(){let $this=$(this),img=$this.find('img');img.each(function(i){i++
let yVal=(e.velocity)/1.5*i;if(i>1){if(horizontal){gsap.to($(this),{x:-1*yVal,ease:'power3.out',})}else{gsap.to($(this),{y:-1*yVal,ease:'power3.out',})}}})})})})}
...
function zoomIn(active){let state=Flip.getState(projects,{props:'opacity, filter'}),tl=gsap.timeline(),left=positions[active.data('index')][1]-25,top=positions[active.data('index')][0]-25;projects.removeClass('active');active.addClass('active')
if(!mobileQuery.matches){gsap.set(wrapper,{x:-1*left,y:-1*top})}
...
function showcaseCards(){let cards=$('.showcase-cards');cards.each(function(){let cards=$(this),overlay=cards.find('.sc-overlay'),wrapper=cards.children('.ssv-projects-wrap'),projects=wrapper.find('.showcase-project'),colorAnim;cards.hasClass('animate-colors')?colorAnim=true:'';projects.first().addClass('active')
projects.each(function(i){let $this=$(this),image=$this.find('.project-image');$this.attr('data-index',i);$this.addClass('project_'+i);gsap.set($this,{zIndex:100-i,y:-i*30,z:-i*100,})
gsap.set(image,{opacity:1-(i/5)})
colorAnim?projectColorSettings($this):'';})
layoutChange(overlay,$('.project_0'),cards)
var wheeler=Hamster(document.querySelector('.showcase-cards')),direction,proj,isAnimating;isAnimating=false;wheeler.wheel(function(event,delta,deltaX,deltaY){direction=deltaY;if((direction==1)&&(!isAnimating)){proj=$('.active');layoutChange('.sc-overlay',proj.next(),cards)
let nextProjects=proj.nextAll(),tl=gsap.timeline({onUpdate:()=>{isAnimating=true},onComplete:()=>{setTimeout(function(){isAnimating=false},500)}});if(proj.hasClass('active')){tl.to(proj,{yPercent:100,duration:1,onComplete:()=>{proj.removeClass('active');proj.next().addClass('active')}},0)
nextProjects.each(function(i){let nexto=$(this),img=nexto.find('.project-image');tl.to(nexto,{y:-i*30,z:-i*100,duration:1,},0)
tl.to(img,{duration:1,opacity:1-(i/5)},0)})}}else if((direction==-1)&&(!isAnimating)){proj=$('.active').prev();layoutChange('.sc-overlay',proj,cards)
let prevProjects=proj.nextAll();if(proj){gsap.to(proj,{yPercent:0,duration:1,onComplete:()=>{proj.addClass('active');proj.next().removeClass('active')}})
prevProjects.each(function(i){i++
let nexto=$(this),img=nexto.find('.project-image');gsap.to(nexto,{y:-i*30,z:-i*100,duration:1,})
gsap.to(img,{opacity:1-(i/5),duration:1,})})}}});})}
...
function naylaLightboxGallery(gallery,items){let $items=$(items),$gallery=$(gallery),itemsLenght=$items.length,thumbsDragger,thumbsWheeler;const $lightbox=$('<div>',{class:'nayla-lightbox-hold',html:`
                    ...
                    <div class="lightbox-close">CERRAR</div>
...
let itemIndex=$(this).data('index');$gallery.addClass('lightbox-convert');$items.addClass('lightbox-thumb');const result=$items.toArray().reduce((a,item)=>a+Math.floor($(item).outerWidth()),10);gsap.to('.active-hold',{width:$this.outerWidth()-9,duration:1})
...
Flip.from(itemsNewState,{duration:2,ease:'expo.inOut',absolute:true,onComplete:()=>{gsap.set(body,{clearProps:'cursor'})}});}}
...
let $this=$(this);if(grid.hasClass('parallax-on')&&$this.find('img').length){$this.find('img').wrap('<div class="parallax-wrap"></div>');$this.find('.parallax-wrap').css('width',$this.outerWidth());$this.find('img').css('height','calc(100% + 100px)');gsap.to($this.find('img'),{y:-100,ease:'none',scrollTrigger:{trigger:$this,scrub:1,start:'top bottom',emd:'bottom top',}})
...
return()=>{}});})}
function naylaSections(){var section=$('#content').find('.section'),page=$('.page-content'),pageBg=page.css('background-color'),pinned;function changeBg(section,color,leave){gsap.to(page,{backgroundColor:color,duration:.6,});section.addClass('active')
...
function naylaVideo(vid){var video=$(vid);if(video.length){video.each(function(i){i++
...
naylaVid.loop=nloop
if(nautoplay==true){naylaVid.play();}}
naylaVid.on('ready',(event)=>{if($this.find('video').attr('autoplay')){naylaVid.play()}
$this.find('.plyr').addClass('nayla_video_'+i);let vid=$('.nayla_video_'+i);vid.find('.plyr__video-wrapper').addClass('nayla-video-wrap');vid.find('.plyr__controls').addClass('nayla-controls-wrap');vid.find('.plyr__control[data-plyr="play"]').append('<span class="hover-default nayla-player-control nayla-play">'+playText+'</span><span class="hover-default nayla-player-control nayla-pause">'+pauseText+'</span>')
vid.find('.plyr__controls__item.plyr__volume button[data-plyr="mute"]').append('<span class="hover-default nayla-player-control nayla-mute">'+muteText+'</span><span class="hover-default nayla-player-control nayla-unmute">'+unmuteText+'</span>');if($this.hasClass('nayla-play')){if(play!=='icon'){$this.append('<div class="play-icon text"><span>'+play+'</span></div>')}else{$this.append('<div class="play-icon icon"><span class="material-icons">play_arrow</span></div>')}
$this.find('.play-icon').on('click',function(){naylaVid.muted=false;naylaVid.restart();$this.addClass('nayla-play-started');})}
function lightboxPlay(){naylaVid.fullscreen=false
vid.append('<span class="lightbox-close">Close</span>')
vid.addClass('nlv').prependTo('#page');$('.nlv').addClass('lightbox-open')
vid.find('.plyr__video-wrapper').addClass('nayla-video-wrap');vid.find('.plyr__controls').addClass('nayla-controls-wrap');$this.find('.play-icon').on('click',function(){vid.addClass('lightbox-open');naylaVid.muted=false;naylaVid.restart();})
$('.lightbox-close').on('click',function(){vid.off('click')
vid.removeClass('nlv lightbox-open');vid.find('.plyr__video-wrapper').removeClass('nayla-video-wrap');vid.find('.plyr__controls').removeClass('nayla-controls-wrap');naylaVid.muted=true;vid.prependTo($this);})}
if($this.hasClass('cursor-play')){vid.addClass('not-started')
naylaVid.controls=false
vid.on('mouseenter',function(){if(vid.hasClass('not-started')){if(play!=='icon'){mouseCursor.find('.mouse-cursor-text').children('span').html(play)
mouseCursor.addClass('hover-text');mouseCursor.addClass('hover-size')
mouseCursor.addClass('hover-player');}else{mouseCursor.find('.mouse-cursor-icon').attr('data-icon','play_arrow')
mouseCursor.addClass('hover-icon');}
mouseCursor.addClass('hover-size')
mouseCursor.addClass('hover-player');}})
vid.on('click',function(){lightboxPlay()
if(vid.hasClass('not-started')){vid.removeClass('not-started')
naylaVid.muted=false;naylaVid.restart();mouseCursor.removeClass('hover-icon');mouseCursor.removeClass('hover-size')
mouseCursor.removeClass('hover-player');mouseCursor.removeClass('hover-text');}})
vid.on('mouseleave',()=>{mouseCursor.removeClass('hover-size')
mouseCursor.removeClass('hover-icon');mouseCursor.removeClass('hover-player');mouseCursor.removeClass('hover-text');})
if($this.hasClass('lightbox')){naylaVid.on('controlshidden',()=>{mouseCursor.addClass('temp_hidden')
mouseCursor.addClass('temp_hidden')
$('.lightbox-close').addClass('temp_hidden')})
naylaVid.on('controlsshown',()=>{mouseCursor.removeClass('temp_hidden');$('.lightbox-close').removeClass('temp_hidden')})}}
...
whaler.wheel(function(event,delta,deltaX,deltaY){wheelDeltaY=event.deltaY;event.deltaY<0?wheelDeltaY=-1*event.deltaY:'';tl.timeScale(1+(wheelDeltaY*2))});createWheelStopListener(window,function(){tl.timeScale(1)});$this.on('click',function(){gsap.to(window,{scrollTo:dataTarget,ease:"power3.out",duration:3})})})}
...
loop();};let resizeThrottle;const reset=()=>{window.addEventListener('resize',()=>{window.clearTimeout(resizeThrottle);resizeThrottle=window.setTimeout(()=>{window.clearTimeout(loopTimeout);setup();},200);},false);};const init=(()=>{canvas=document.getElementById('noised-bg');ctx=canvas.getContext('2d');setup();})();};$('#noised-bg').length?noise():'';function naylaScrollButtons(){let button=$('.scroll-button');button.each(function(){let $this=$(this),target=$this.data('scroll-to');$this.on('click',()=>{setTimeout(function(){gsap.to(window,{duration:1,scrollTo:isNaN(target)?$(target).offset().top-25:target,ease:'expo.inOut',});},100)})})}
...
naylaServices()
naylaNumberCounter();naylaAccordion();naylaInfiniteTabs();naylaClinetsCarousel();naylaInfiniteCarousel();naylaDynamicGrid();naylaForms();gridScroll()
naylaTestimonials()
...
if(($('.page-loader').length)&&(!sessionStorage.getItem('doNotShow'))){sessionStorage.setItem('doNotShow','true');setTimeout(function(){pageLoader()},200)}else{$('.page-loader').remove()}
naylaMouseCursor(true);smoothScroll();$(window).on('load',function(){winLoaded=true;window.scrollTo(0,0);naylaVideo($('.nayla-video'));naylaGeneralAnims($('.has-anim'));naylaTextAnims();naylaListAnimations();naylaImageAnims()
naylaParallaxImages();initShowcases();initPageElements();naylaSections();initPages();naylaHeader();naylaScrollButtons()
document.querySelector('.page-loader')?ScrollTrigger.disable():null;ScrollTrigger.refresh(true);})
let caption=$('.page-transition-caption');new SplitText(caption,{type:'lines, chars',linesClass:'capt_lines',charsClass:'capt_chars'})
function transitionCaption(tl,intro,outro){let chars=$('.capt_chars');if(intro){tl.from(chars,{yPercent:100,duration:1.5,ease:'expo.out',delay:.4,stagger:0.025},0)}
if(outro){tl.to(chars,{yPercent:-100,duration:.5,ease:'expo.in',stagger:-0.015,onComplete:()=>{gsap.set(chars,{clearProps:'y'})}},0)}}
let transitions=$('.nayla-page-transition'),block,paper,overlay,up,down,left,right,opacity,pageOver;transitions.hasClass('up')?up=true:transitions.hasClass('down')?down=true:transitions.hasClass('left')?left=true:transitions.hasClass('right')?right=true:'';transitions.hasClass('block')?block=true:transitions.hasClass('overlay')?overlay=true:transitions.hasClass('opacity')?opacity=true:transitions.hasClass('page-over')?pageOver=true:'';function blockTrans(){let blocksCount=Math.ceil($(window).outerHeight()/75),i=0;for(i=0;i<=blocksCount;i++){transitions.append('<span class="transition-block"></span>')}}
block?blockTrans():'';function pageOverDef(){$('<span class="page-over-ovs"></span>').insertBefore(transitions)}
pageOver?pageOverDef():'';function animateBlocks(time,intro,outro){let direction=left?'inset(0% 100% 0% 0%)':right?'inset(0% 0% 0% 100%)':up?'inset(100% 0% 0% 0%)':down?'inset(0% 0% 100% 0%)':'',stagger=up?-0.04:down?0.04:left?0.04:right?-0.04:'',ease=up?'power4.out':down?'power4.out':left?'power4.inOut':right?'power4.inOut':'',duration=up?.65:down?.65:left?1:right?1:'';if(intro){transitions.css('visibility','visible');time.to('.transition-block',{clipPath:'inset(0% 0% 0% 0%)',duration:duration,ease:ease,stagger:stagger})}else if(outro){time.to('.transition-block',{clipPath:direction,duration:duration,ease:ease,stagger:stagger})}}
function animateOverlay(time,intro,outro){if(intro){time.to(transitions,{height:'100vh',width:'100vw',duration:1,ease:'power3.inOut'},0)}
if(outro){let top=up?0:down?'unset':'',bottom=up?'unset':down?0:'',height=up?'0vh':down?'0vh':left?'100vh':right?'100vh':'',width=up?'100vw':down?'100vw':left?'0vh':right?'0vh':'',lefto=left?'unset':right?0:'',righto=left?0:right?'unset':'';time.to(transitions,{height:height,width:width,duration:1,left:lefto,right:righto,top:top,bottom:bottom,ease:'power3.inOut'},0)}}
function animateOpacity(time,intro,outro){if(intro){time.to('#content, .project-page-header',{y:up?-100:down?100:0,x:left?100:right?-100:0,opacity:0,duration:1,ease:'power3.in',},0)}
if(outro){time.from('#content, .project-page-header',{y:up?100:down?-100:0,x:left?-100:right?100:0,opacity:0,duration:1,ease:'power3.out',},0)}}
...
function getMediaToCenter(){var media=document.querySelector('.transition--media')
let state=Flip.getState(media,{props:'top, left, x, y'}),height=media.offsetHeight,width=media.offsetWidth;gsap.set(media,{rotate:0,position:'fixed',height:height,width:width,top:window.innerHeight/2,left:window.innerWidth/2,xPercent:-50,yPercent:-50,x:0,y:0,rotate:0,opacity:1,clipPath:'inset(0%)',display:'block',maxHeight:'unset',maxWidth:'unset'})
Flip.from(state,{duration:1,absolute:true,absoluteOnLeave:true,});}
function getMediaPosition(){var media=document.querySelector('.transition--media')
let state=Flip.getState(media,{props:'top, left, x, y'}),height=media.offsetHeight,width=media.offsetWidth;gsap.set(media,{rotate:0,position:'fixed',height:height,width:width,top:window.innerHeight/2,yPercent:-50,})
Flip.from(state,{duration:1,absolute:true,absoluteOnLeave:true,});}
...
function projectBackground(current,next,tl){if(current===next){}else{let ov=document.createElement('span')
ov.classList.add('project-trans-overlay');ov.style.backgroundColor=next;document.querySelector('#page').append(ov);tl.to(ov,{height:'100svh',duration:1,ease:'expo.inOut',onStart:()=>{setTimeout(function(){headerLayoutChange(false,next,false,false)},650)}},1.75)}}
...
tl.to([slides,'.sfc-fraction','.showcase-footer','.project-button'],{opacity:0},0)}
function fullscreenCarouselIn(tl){tl.to('.fc_char',{yPercent:-100,duration:.5,stagger:0.05,ease:'power3.in'})}
...
function outAnimShowcaseCarousel(time){let carousel=$('.showcase-carousel'),project=carousel.find('.showcase-project.active'),title=carousel.find('.project-title'),image=carousel.find('.swiper-slide-active .slide-bgimg');image.addClass('trans-image')
carousel.addClass('animating');time.to(title,{yPercent:-100,duration:1,ease:'expo.inOut'},0)
time.to([$('.showcase-footer'),$('.sc-lines')],{opacity:0,duration:1,ease:'expo.inOut'},0)}
function minimalListOut(tl){tl.to('.sml-wrapper .project-title',{yPercent:-100,duration:1,ease:'power4.in',stagger:0.025},0)}
function fullscreenWallOut(tl){tl.to('.title_line',{yPercent:-25,rotateX:-90,duration:1,ease:'expo.in'},0)
tl.to('.project-title, .showcase-footer',{opacity:0,duration:1,ease:'expo.in'},0)}
function showcase3dTitlesOut(tl){let currRot=$('.s3t-projects-wrap').css('transform');tl.to('.s3t-projects-wrap .project-title',{y:-200,duration:1,ease:'expo.in',opacity:0},0)
tl.to('.showcase-footer',{opacity:0},0)}
barba.init({debug:false,cacheIgnore:true,transitions:[{name:'default-transition',leave(){return new Promise(function(resolve,reject){transitions.addClass('running')
let tl=gsap.timeline({onComplete:()=>{resolve();}});block?animateBlocks(tl,true,false):overlay?animateOverlay(tl,true,false):opacity?animateOpacity(tl,true,false):pageOver?animatePageOver(tl,true,false):'';transitionCaption(tl,true,false)})},beforeEnter(){return new Promise(function(resolve,reject){let tl=gsap.timeline({onStart:()=>{resolve();transitions.removeClass('running')},onComplete:()=>{gsap.set(transitions,{clearProps:'all'})
if(block){gsap.set('.transition-block',{clearProps:'all'})}}});block?animateBlocks(tl,false,true):overlay?animateOverlay(tl,false,true):opacity?animateOpacity(tl,false,true):pageOver?animatePageOver(tl,false,true):'';transitionCaption(tl,false,true)})}},{name:'fullscreen-menu-transition',from:{custom:({trigger})=>{return trigger.classList&&trigger.classList.contains('menu-link');},},leave(){return new Promise(function(resolve,reject){transitions.addClass('running')
let tl=gsap.timeline({onComplete:()=>{resolve();}}),toggle=$('.menu-toggle');toggle.trigger('click')
gsap.getTweensOf('.menu-overlay')[0].kill();gsap.set('.nayla-page-transition',{height:'100vh',width:'100vw',background:'transparent',transitionDuration:'0s'})
transitionCaption(tl,true,false)})},beforeEnter(){return new Promise(function(resolve,reject){let tl=gsap.timeline({onStart:()=>{resolve();transitions.removeClass('running')},onComplete:()=>{gsap.set(transitions,{clearProps:'all'})}}),overlay=$('.menu-overlay');tl.to(overlay,{height:0,duration:.7,ease:'expo.inOut',onComplete:()=>{$('.menu-toggle').removeClass('active');$('#site-navigation').removeClass('active');}},.5)
transitionCaption(tl,false,true)})}},{name:'media-center-to-image',from:{custom:({trigger})=>{return trigger.classList&&trigger.classList.contains('sc-link');},},to:{namespace:['project-half-image','project-gallery-horizontal','project-no-image','project-gallery-vertical','project-video','project-creative']},leave(){return new Promise(function(resolve,reject){let tl=gsap.timeline({onComplete:()=>{resolve()}})
getMediaToCenter()
$('div.showcase-carousel').length?outAnimShowcaseCarousel(tl):$('div.showcase-fullscreen-carousel').length?fullscreenCarouselOut(tl):$('div.showcase-fullscreen-slideshow').length?fullscreenSlideshowOut(tl):$('div.showcase-minimal-list').length?minimalListOut(tl):$('div.showcase-fullscreen-wall').length?fullscreenWallOut(tl):$('div.showcase-list').length?showcaseListOut(tl):$('div.showcase-3d-titles').length?showcase3dTitlesOut(tl):null;})},beforeEnter(){return new Promise(function(resolve,reject){let tl=gsap.timeline({onComplete:()=>{resolve()
document.querySelector('.project-trans-overlay')?document.querySelector('.project-trans-overlay').remove():null;}})
let next=$('.project-page-header').css('backgroundColor'),current=$('.sfc-overlay').css('backgroundColor')
projectBackground(current,next,tl)
animateMedia(tl,false,false,true)
$('div.showcase-fullscreen-carousel').length?fullscreenCarouselIn(tl):null;})}},{name:'media-center-to-full-image',from:{custom:({trigger})=>{return trigger.classList&&trigger.classList.contains('sc-link');},},to:{namespace:['project-fullscreen-image','project-tall']},leave(){return new Promise(function(resolve,reject){let tl=gsap.timeline({delay:1,onComplete:()=>{resolve()}})
getMediaToCenter();$('div.showcase-carousel').length?outAnimShowcaseCarousel(tl):$('div.showcase-fullscreen-carousel').length?fullscreenCarouselOut(tl):$('div.showcase-fullscreen-slideshow').length?fullscreenSlideshowOut(tl):$('div.showcase-minimal-list').length?minimalListOut(tl):$('div.showcase-fullscreen-wall').length?fullscreenWallOut(tl):$('div.showcase-list').length?showcaseListOut(tl):$('div.showcase-3d-titles').length?showcase3dTitlesOut(tl):null;})},beforeEnter(){return new Promise(function(resolve,reject){let tl=gsap.timeline({onComplete:()=>{resolve()}})
animateMedia(tl,true)
$('div.showcase-fullscreen-carousel').length?fullscreenCarouselIn(tl):null;})}},]})
if(history.scrollRestoration){history.scrollRestoration="manual";}
ScrollTrigger.clearScrollMemory('manual');barba.hooks.before((data)=>{$('html').addClass('loading');disableScroll();$('#mouseCursor').removeClass('hover-size hover-text hover-icon')});barba.hooks.enter((data)=>{var response=data.next.html.replace(/(<\/?)body( .+?)?>/gi,'$1notbody$2>',data.next.html),bodyClasses=$(response).filter('notbody').attr('class'),bodyStyles=$(response).filter('notbody').attr('style'),headerClasses=$(response).filter('notbody').find('#header').attr('class'),headerInner=$(response).filter('notbody').find('#header').html();if($('body').hasClass('smooth-scroll')){let aliothSmoother=ScrollSmoother.get();aliothSmoother.scrollTo(0,false);window.scrollTo(0,0);}else{window.scrollTo(0,0);}
if(siteHeader.hasClass('menu-open')){setTimeout(function(){siteHeader.attr('class',headerClasses)},1000)}else{siteHeader.attr('class',headerClasses)}
$('body').attr('class',bodyClasses);let Alltrigger=ScrollTrigger.getAll()
...
enableScroll();naylaTextAnims()
naylaGeneralAnims($('main .has-anim'));naylaListAnimations();naylaImageAnims()
naylaParallaxImages();initShowcases();initPageElements();naylaSections();initPages();naylaVideo($('#primary').find('.nayla-video'));ScrollTrigger.refresh(true);naylaMouseCursor(false);setTimeout(function(){let allInstances=ScrollTrigger.getAll();let fixedHeader=ScrollTrigger.getById('fixed_header');allInstances.forEach(function(st){if((st.pin!=null)&&(st.start<=0)){let fixedStart='top+='+st.end+' top';ScrollTrigger.create({trigger:'body',pin:siteHeader,start:'top top',end:fixedStart,id:'header_pin'})}})
ScrollTrigger.refresh(true);},50)});}(jQuery));